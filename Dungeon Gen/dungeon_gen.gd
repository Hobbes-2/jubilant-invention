...
func set_seed(val : String) -> void:
	...
	seed(val.hash())
func _ready() -> void:
	#player_controller.position = room_positions[0]
	#start = true
...
@export var room_dist : int = 2
@export var room_retry : int = 15
...
func make_room(rec : int):
	...
	#make the room
	for r in range(-room_dist, height + room_dist):
		for c in range(-room_dist, width + room_dist):
			#change the middle value to change the height of the rooms
	...
	var average_z : float = start_pos.z + (float(height)/2)
	var pos : Vector3 = Vector3(average_x, 0, average_z)
